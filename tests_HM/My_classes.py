...
class Car:
    ...
    def json_car_loads(self, our_dict):
        return json.loads(our_dict, object_hook=self.dict_to_object_car())
...
class Garage:
    ...
    def convert_to_dict_garage(self):
        ser_car = []
        obj_dict = {"__class__": self.__class__.__name__, "__module__": self.__module__}
        obj_dict.update(self.__dict__)
        for car in self.cars:
            car = Car.convert_to_dict_car(car)
            ser_car.append(car)
        obj_dict.update({'cars': ser_car})
        return obj_dict

    def garage_dumps(self):
        garage_dict = self.convert_to_dict_garage()
        return json.dumps(garage_dict, indent=4)

    def garage_dump(self):
        with open('my_json_garage', 'w') as file:
            json.dump(self.convert_to_dict_garage(), file, indent=4)

    # def garage_load(self):
    # def garage_loads(self):
...
class Collecionar:
    ...
    def convert_to_dict_collectioner(self):
        obj_dict = {"__class__": self.__class__.__name__, "__module__": self.__module__}
        obj_dict.update(self.__dict__)
        obj_dict['register_id'] = str(obj_dict['register_id'])

        for garage in obj_dict['garages']:
            garage = Garage.convert_to_dict_garage(garage)
            obj_dict.update(({'garages': garage}))
        return obj_dict

    def collectionar_dumps(self):
        collectionar = self.convert_to_dict_collectioner()
        return json.dumps(collectionar, indent=4)

    def collectionar_dump(self):
        with open('my_json_collectionar', 'w') as file:
            json.dump(self.convert_to_dict_collectioner(), file, indent=4)

    # def collectionar_loads(self):
    # def collectionar_load(self):
    def __eq__(self, other):
...
collectionar_1 = Collecionar('Jshua')
collectionar_2 = Collecionar('Myhamed')
my_json_car = car_2.json_car_dumps()
# print(my_json_car)
# print(type(my_json_car))
# print(my_json_car.json_car_loads())

# print(collectionar_2.collectionar_dumps())
# print(garage_2.garage_dumps())
